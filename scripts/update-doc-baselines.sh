...
SCHEMA_VERSION=$(grep 'pub const CURRENT_SCHEMA_VERSION' crates/cc-db/src/index_migrate.rs | grep -o '[0-9]*')
...
TOTAL_IGNORED=$(echo "$TEST_OUTPUT" | grep '^test result:' | awk '{sum+=$10} END {print sum}')
...
EVAL_IGNORED=$(echo "$TEST_OUTPUT" | grep '^test result:' | sed -n '2p' | awk '{print $10}')
...
EXPECTED_TOTAL=$(head -5 docs/TEST_PLAN.md | grep -o '[0-9]* tests' | grep -o '[0-9]*')
...
if [ "$TOTAL" != "$EXPECTED_TOTAL" ] 2>/dev/null; then
    echo ""
    echo "DRIFT: total test count changed: doc=${EXPECTED_TOTAL} actual=${TOTAL}"
    DRIFT=1
fi
