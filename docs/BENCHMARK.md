...
| Incremental reindex | <2s for 10 changed files | Measure via `index(path)` after edits (file watcher not yet integrated) |
...
This executes a 3-run warm benchmark (1 warmup + 2 measured, take min) and writes results to `docs/benchmarks/latest.md`. Without `CODECORTEX_WRITE_BENCHMARK`, the test still runs but does not persist the report.
